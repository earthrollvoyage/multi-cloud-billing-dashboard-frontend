...
import DashboardPieChartAnalytics from "../../../components/chart/pieChart/dashboard/azure";
import StackedBarChartAnalytics from "../../../components/chart/stackedBarChart/dashboard/azure";
import DashboardBarChartAnalytics from "../../../components/chart/barChart/dashboard/azure";
import TableContent from "../../../components/tableContent";
import { ChartSelectContext } from "../../../data/chartSelectContext";
import DashboardLineChartAnalytics from "../../../components/chart/lineChart/dashboard/azure";
...
const AZUREBillingDashboard = ({
  ...
  const [azureFilterDataSet, setAzureFilterDataSet] = useState({
  ...
  const handleChangeAzureFilter = (valueSelect) => {
    setAzureFilterDataSet(valueSelect);
  };

  const chartSelectContext = { toggleChart, handleChange };
  const azureFilterDataContext = { azureFilterDataSet, handleChangeAzureFilter };

  return (
    <AzureFilterDataContext.Provider value={azureFilterDataContext}>
      <ChartSelectContext.Provider value={chartSelectContext}>
        <DashboardContainer>
          ...
          <NavigatorWrap>
            ...
            <NavigatorItemWord>Azure Cost Analysis</NavigatorItemWord> 
          </NavigatorWrap>
          <TopTitle>
            <Title>
              {/* <MainTitle>
              ...
              <MainTitle>|</MainTitle>
              <SubTitle>Azure Cost Analysis</SubTitle>
            </Title>
          </TopTitle>
          <DashboardWrapper>
            ...
            <ChartContainer>
                ...
                <ChartContainerLeftTop>
                  {toggleChart === "line" ? (
                    <DashboardLineChartAnalytics
                      title={ azureFilterDataSet.groupBy === "service_family" ? "Cost by Service Family" 
                      : azureFilterDataSet.groupBy === "billing_profile_name" ? "Cost by Billing Profile Name"
                      : azureFilterDataSet.groupBy === "subscription_name" ? "Cost by Subscription Name"
                      : "Cost by Service Name" }
                      filterDataSet={azureFilterDataSet}
                      page={page}
                      data={data}
                      dataKey="month"
                      dataValueKeys="Costs"
              ...
              <ChartContainerRight>
                ...
                <DashboardPieChartAnalytics
                  title={ azureFilterDataSet.groupBy === "service_family" ? "Spend by Service Family" 
                  : azureFilterDataSet.groupBy === "billing_profile_name" ? "Spend by Billing Profile Name"
                  : azureFilterDataSet.groupBy === "subscription_name" ? "Spend by Subscription Name"
                  : "Spend by Service Name" }
                  filterDataSet={azureFilterDataSet}
                  page={page}
                  cx="50%"
                  cy="50%"
    ...
    </AzureFilterDataContext.Provider>
  );
};
