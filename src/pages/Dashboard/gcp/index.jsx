...
import DashboardPieChartAnalytics from "../../../components/chart/pieChart/dashboard/gcp";
import StackedBarChartAnalytics from "../../../components/chart/stackedBarChart/dashboard/gcp";
import DashboardBarChartAnalytics from "../../../components/chart/barChart/dashboard/gcp";
import TableContent from "../../../components/tableContent";
import { ChartSelectContext } from "../../../data/chartSelectContext";
import DashboardLineChartAnalytics from "../../../components/chart/lineChart/dashboard/gcp";
...
const GCPBillingDashboard = ({
  ...
  const [gcpFilterDataSet, setGCPFilterDataSet] = useState({
    dateRange: { startDate: startDateDefault, endDate: endDateDefault },
    granularity: 'monthly',
    groupBy: 'project_cost',
    listSelectBy: [],
    pieChart: {
      top_five_cost: [
  ...
  const handleChangeGCPFilter = (valueSelect) => {
    setGCPFilterDataSet(valueSelect);
  };

  const chartSelectContext = { toggleChart, handleChange };
  const gcpFilterDataContext = { gcpFilterDataSet, handleChangeGCPFilter };

  return (
    <GCPFilterDataContext.Provider value={gcpFilterDataContext}>
      <ChartSelectContext.Provider value={chartSelectContext}>
        <DashboardContainer>
          ...
          <NavigatorWrap>
            ...
            <NavigatorItemWord>GCP Cost Analysis</NavigatorItemWord> 
          </NavigatorWrap>
          <TopTitle>
            <Title>
              {/* <MainTitle>
              ...
              <MainTitle>|</MainTitle>
              <SubTitle>GCP Cost Analysis</SubTitle>
            </Title>
          </TopTitle>
          <DashboardWrapper>
            ...
            <ChartContainer>
                ...
                <ChartContainerLeftTop>
                  {toggleChart === "line" ? (
                    <DashboardLineChartAnalytics
                      title={ gcpFilterDataSet.groupBy === "project_cost" ? "Cost by Project" : "Cost by Service" }
                      filterDataSet={gcpFilterDataSet}
                      page={page}
                      data={data}
                      dataKey="month"
                      dataValueKeys="Costs"
              ...
              <ChartContainerRight>
                ...
                <DashboardPieChartAnalytics
                  title={ gcpFilterDataSet.groupBy === "project_cost" ? "Spend by Project" : "Spend by Service" }
                  filterDataSet={gcpFilterDataSet}
                  page={page}
                  cx="50%"
                  cy="50%"
    ...
    </GCPFilterDataContext.Provider>
  );
};

export default GCPBillingDashboard;
