...
import { getRankingAzureCost } from "../../../../../services/billing/costAnalysis/azure/getRankingCost";
...
const DashboardPieChartAnalytics = ({
  ...
  const [groupBy, setGroupBy] = useState({ 'service_family': "true", 'service_name': "false" });
  ...
  useEffect(() => {
    ...
    let groupByKey = groupBy.service_family === "true" ? 'service_family' : 'service_name'
    
    getRankingAzureCost(startDateDefault, endDateDefault, groupByKey).then((res) => {
      setRankingCosts(res)
      setDataFetched(true);
    });
  ...
  return (
    <ChartContainer marginRightValue={marginRightValue}>
      ...
      <ChartTitleWrap>
        <ChartTitleInnerLeft>{groupBy.service_family === 'true' ? 'Spend by Project' : 'Spend by Service'}</ChartTitleInnerLeft>
        <ChartTitleInnerRight>
          <RankingButtonComponent title="Project" page={page} groupby={groupBy.service_family} handleOnChange={handleOnChange} />
          <RankingButtonComponent title="Serivce" page={page} groupby={groupBy.service_name} handleOnChange={handleOnChange} />
        </ChartTitleInnerRight>
      </ChartTitleWrap>
