...
import { getRankingGCPCost } from "../../../../../services/billing/costAnalysis/gcp/getRankingCost";
...
const DashboardPieChartAnalytics = ({
  ...
  const [groupBy, setGroupBy] = useState({ 'project_cost': "true", 'service_cost': "false" });
  ...
  useEffect(() => {
    ...
    let groupByKey = groupBy.project === "true" ? 'project_cost' : 'service_cost'
    
    getRankingGCPCost(startDateDefault, endDateDefault, groupByKey).then((res) => {
      setRankingCosts(res)
      setDataFetched(true);
    });
  ...
  return (
    <ChartContainer marginRightValue={marginRightValue}>
      ...
      <ChartTitleWrap>
        <ChartTitleInnerLeft>{groupBy.project === 'true' ? 'Spend by Project' : 'Spend by Service'}</ChartTitleInnerLeft>
        <ChartTitleInnerRight>
          <RankingButtonComponent title="Project" page={page} groupby={groupBy.project} handleOnChange={handleOnChange} />
